                             ...
                             char *name)
{
    ...
    if (vac->send(vac, (char *)mp, sizeof(*mp) + sizeof(*apath), &out, &out_len))
    {
                         ...
                         int prefix, bool nexthop, char **iface, host_t *src)
{
	...
	mp->_vl_msg_id = htons(VL_API_IP_ROUTE_DUMP);
...
static void update_addrs(private_kernel_vpp_net_t *this, iface_t *entry)
{
    ...
    mp = vl_msg_api_alloc(sizeof(*mp));
	clib_memset(mp, 0, sizeof(*mp));
    mp->_vl_msg_id = htons(VL_API_IP_ADDRESS_DUMP);
    ...
    for (i = 0; i < num; i++)
    {
        rmp = (void *)out;
        out += sizeof(*rmp);
        host = host_create_from_chunk(AF_INET6, chunk_create(rmp->prefix.address.un.ip6, 16), 0);
        addrs->insert_last(addrs, host);
    }
    vl_msg_api_free(mp);
    free(tmp);
 ...
 */
            ...
            {
                entry->up = (event->flags & IF_STATUS_API_FLAG_LINK_UP)? TRUE : FALSE;
